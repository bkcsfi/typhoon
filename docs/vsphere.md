...
Controllers and workers are provisioned to run a `kubelet`. A one-time [bootkube](https://github.com/kubernetes-incubator/bootkube) bootstrap schedules an `apiserver`, `scheduler`, `controller-manager`, and `kube-dns` on controllers and runs `kube-proxy` and `calico` on each node. A generated `kubeconfig` provides `kubectl` access to the cluster.

## Requirements

* Licensed VMWare vSphere (the free version of eSXi does not support API calls)
* vSphere Access Credentials
* CoreOS Template VM
* Terraform v0.11.x and [terraform-provider-ct](https://github.com/coreos/terraform-provider-ct) installed locally

## CoreOS Template VM Setup

Follow the [CoreOS Instructions](https://coreos.com/os/docs/latest/booting-on-vmware.html) to download and deploy the desired
coreos OVA version to vSphere. Stable Channel is recommended.  After deploying the OVA file to vSphere, convert the deployed VM to a Template using the vSphere Client
